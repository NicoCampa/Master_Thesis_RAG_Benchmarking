...
class OllamaModel:
    def __init__(self, model_name='llama3.2:3b', api_url='http://localhost:11434'):
        """
        Initializes the OllamaModel with the specified model name and API URL.

        Args:
            model_name (str): The name of the deployed model in Ollama.
    ...
    def generate(self, text, temperature=0.7, system="", top_p=0.8, max_new_tokens=256):
        """
        Generates a response from the Ollama model based on the input text.

        Args:
            text (str): The user input text or prompt.
            temperature (float): Sampling temperature.
            system (str): Optional system-level instructions or prompts.
        ...
        """
        # Construct the prompt with system instructions if provided
        if system:
            prompt = f"{system}\n\n{text}"
        else:
            prompt = text
        ...
        try:
            ...
            response = requests.post(f"{self.api_url}/v1/generate", json=payload)
