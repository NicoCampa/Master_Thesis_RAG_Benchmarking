...
def main():
    ...
    evaluate_file = os.path.join(
        outputs_dir,
        f"prediction_{args.dataset}_{args.ollama_model}_temp{args.temp}_noise1.0_passage{args.passage_num}_correct0.0.json"
    )

    # Output file with ChatGPT evaluations
    output_file = os.path.join(
        outputs_dir,
        f"prediction_{args.dataset}_{args.ollama_model}_temp{args.temp}_noise1.0_passage{args.passage_num}_correct0.0_chatgpt.json"
    )

    # Final results metrics file
    result_file = os.path.join(
        metrics_dir,
        f"prediction_{args.dataset}_{args.ollama_model}_temp{args.temp}_noise1.0_passage{args.passage_num}_correct0.0_chatgptresult.json"
    )
